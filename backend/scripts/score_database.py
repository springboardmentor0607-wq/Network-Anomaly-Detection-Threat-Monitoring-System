...
MODEL_DIR = r"e:\NetShield\backend\app\models"
...
async def score_database():
    ...
    async for document in cursor:
        ...
        if len(batch) >= BATCH_SIZE:
            await process_batch(batch, collection, iso_forest, xgb, scaler, label_encoder, feature_names, analysis_stats)
...
async def process_batch(batch, collection, iso_forest, xgb, scaler, label_encoder, feature_names, stats):
    ...
    # Clean Data
    # Coerce to numeric, converting non-numeric strings (like '0xc0a8') to NaN
    ...
    # IsolationForest returns 1 for inliers, -1 for anomalies. Map to True/False
    ...
    for i, doc in enumerate(batch):
        ...
        risk_score = conf * 100
        if t_class != "BENIGN":
            risk_score = min(risk_score * 1.5, 99.9)  # Boost attack risk
        if anomaly:
