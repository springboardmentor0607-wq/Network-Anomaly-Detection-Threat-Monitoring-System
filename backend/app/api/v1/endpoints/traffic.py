...
def simulate_traffic(
    ...
    """Simulate network traffic flows for demo purposes."""
    return TrafficService.simulate_traffic_flows(db, count=count)

@router.get("/{flow_id}", response_model=TrafficFlowResponse)
def get_traffic_flow(
    flow_id: str,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)
) -> Any:
    flow = db.query(TrafficFlow).filter(TrafficFlow.id == flow_id).first()
    if not flow:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Traffic flow with ID {flow_id} not found"
        )
