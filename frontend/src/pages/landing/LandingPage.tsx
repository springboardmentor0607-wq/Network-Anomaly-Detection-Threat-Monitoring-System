...
export const LandingPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#000000] text-white relative overflow-x-hidden selection:bg-cyan-950/400 selection:text-black">
      {/* Background Interactive Node Canvas */}
      <canvas ref={canvasRef} className="absolute inset-0 z-0 pointer-events-none opacity-60" />

      {/* Top Header Navbar */}
      <header className="relative z-10 max-w-7xl mx-auto px-6 h-20 flex items-center justify-between border-b border-[#333333]/50">
        ...
        <div className="flex items-center space-x-4">
          ...
          <button
            onClick={() => navigate('/')}
            className="px-5 py-2.5 bg-cyan-950/400 hover:bg-cyan-400 text-black font-bold text-xs rounded-xl shadow-lg shadow-cyan-500/25 transition transform hover:-translate-y-0.5 flex items-center space-x-2"
          >
      ...
      <section className="relative z-10 max-w-5xl mx-auto px-6 pt-24 pb-20 text-center">
        ...
        <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4">
          <button
            onClick={() => navigate('/')}
            className="w-full sm:w-auto px-8 py-4 bg-cyan-950/400 hover:bg-cyan-400 text-black font-bold text-sm rounded-xl shadow-xl shadow-cyan-500/30 transition transform hover:-translate-y-0.5 flex items-center justify-center space-x-3"
          >
            <span>Launch SOC Dashboard</span>
          ...
          <a
            href="#features"
            className="w-full sm:w-auto px-8 py-4 bg-[#121212] hover:bg-[#2A2A2A] border border-[#333333] text-white font-semibold text-sm rounded-xl transition flex items-center justify-center space-x-2"
          >
      ...
      <section id="features" className="relative z-10 max-w-7xl mx-auto px-6 py-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {features.map((feat, idx) => (
            <div
              key={idx}
              className="p-6 rounded-2xl bg-[#121212]/80 border border-[#333333] hover:border-cyan-500/40 backdrop-blur-md transition group duration-200"
            >
              <div className="p-3 bg-[#000000] border border-[#333333] rounded-xl w-fit mb-4 group-hover:scale-110 transition transform">
                {feat.icon}
              </div>
      ...
      <section className="relative z-10 max-w-5xl mx-auto px-6 py-16">
        <div className="p-8 rounded-3xl bg-[#000000] border border-[#333333] text-center shadow-2xl">
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 text-xs font-semibold">
            {['Network Traffic', 'Feature Extraction', 'ML Anomaly Detection', 'Intrusion Prediction', 'Risk Scoring (0-100)', 'Threat Alerts', 'Incident Response'].map((step, idx) => (
              <React.Fragment key={idx}>
                <span className="px-3 py-2 bg-[#000000] border border-[#333333] rounded-xl text-cyan-300">{step}</span>
                {idx < 6 && <span className="text-gray-400">→</span>}
              </React.Fragment>
            ))}
          </div>
        </div>
      ...
      {/* Footer */}
      <footer className="relative z-10 border-t border-[#333333] py-8 text-center text-xs text-gray-500">
