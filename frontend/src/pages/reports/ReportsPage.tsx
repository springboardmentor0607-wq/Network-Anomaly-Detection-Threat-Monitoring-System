...
export const ReportsPage: React.FC = () => {
  ...
  const handleDownload = (format: 'pdf' | 'csv') => {
    if (format === 'pdf') {
      const doc = new jsPDF();
      doc.setFontSize(16);
      doc.text("NETSHIELD AI SECURITY REPORT", 20, 20);
      doc.setFontSize(12);
      doc.text(`Title: ${selectedReport.title}`, 20, 30);
      doc.text(`ID: ${selectedReport.id}`, 20, 40);
      doc.text(`Generated: ${new Date(selectedReport.generatedAt).toLocaleString()}`, 20, 50);
      doc.text(`Author: ${selectedReport.author}`, 20, 60);
      
      doc.setFontSize(14);
      doc.text("Metrics:", 20, 75);
      doc.setFontSize(12);
      doc.text(`- Threat Anomalies: ${selectedReport.threats}`, 25, 85);
      doc.text(`- Critical Incidents: ${selectedReport.incidents}`, 25, 95);
      doc.text(`- Average Risk Score: ${selectedReport.avgRisk}/100`, 25, 105);
      
      doc.setFontSize(14);
      doc.text("Executive Summary:", 20, 120);
      doc.setFontSize(11);
      
      const splitText = doc.splitTextToSize(selectedReport.summary, 170);
      ...
      a.href = url;
      a.download = `${selectedReport.id}.csv`;
      a.click();
          ...
          {reports.map((rpt) => (
            ...
            >
              <div className="flex items-center justify-between text-xs mb-1">
                ...
                <span className="px-2 py-0.5 bg-[#000000] border border-[#333333] text-gray-300 rounded text-[10px] font-bold">
                  {rpt.type}
                </span>
              </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card title={`Security Report Preview: ${selectedReport.id}`}>
            <div className="space-y-6">
                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex space-x-2">
                    <button
                      ...
                      className="px-3 py-1.5 bg-cyan-950/400 hover:bg-cyan-400 text-black font-bold text-xs rounded-lg shadow transition flex items-center space-x-1"
                    >
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 bg-[#000000] hover:bg-[#2A2A2A] border border-[#333333] text-gray-200 font-semibold text-xs rounded-lg transition flex items-center space-x-1"
                    >
                      <Download className="w-3.5 h-3.5" />
              ...
              <div className="grid grid-cols-3 gap-4 text-center bg-[#000000] p-4 rounded-xl border border-[#333333] text-xs">
              ...
              <div>
                ...
                <p className="text-xs text-gray-300 leading-relaxed bg-[#000000] p-4 rounded-xl border border-[#333333]">
                  {selectedReport.summary}
                </p>
              </div>
            </div>
